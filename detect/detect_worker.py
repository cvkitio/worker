...
class DetectWorker:
    ...
    def load(self):
        ...
        logger.info(f"Loaded configuration for DetectWorker. PID: {os.getpid()}")

    def run(self):
        self.load()
        logger.info(f"DetectWorker started, waiting for items in the queue... PID: {os.getpid()}")
        while True:
            if not self.queue.empty():
                frame_data: Frame = self.queue.get()
                logger.info(f"{os.getpid()} Processing item from queue: {frame_data.detector}")
                ...
                match frame_data.detector:
                    case "face_detector":
                        ...
                        logger.info(f"Detected faces: {len(faces)}. PID: {os.getpid()}")
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    logger.info(f"DetectWorker PID {os.getpid()} received quit signal.")
                    break
            else:
                time.sleep(0.1)