...
def main():
    ...
    with Manager() as manager:
        ...
        # TODO this is currently for one frame, we should make it the same size as the target frame
        shm = shared_memory.SharedMemory(create=True, size=1024 * 1024 * 1024)  # 1MB shared memory
        logger.info(f"Shared memory created with name {shm.name} and size {shm.size}")
        ...
        with ProcessPoolExecutor() as executor:
            logger.info(f"Spawning FrameWorker and {num_workers} DetectWorkers. PID: {os.getpid()}")
